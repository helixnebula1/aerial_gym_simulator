...
class StateQClass:
    ...
    def nextState(self):
        ...
        # Double check the order these go in the queue and are used in teh dqn # TODO
        rel_depth_images = torch.stack(
            [self.depth.queue[1], self.depth.queue[2], self.depth.queue[3]], dim=1)

        # Stack the most recent 8 positions
        rel_pos = torch.stack([self.pos.queue[1], self.pos.queue[2],
                               self.pos.queue[3], self.pos.queue[4],
                               self.pos.queue[5], self.pos.queue[6],
                               self.pos.queue[7], self.pos.queue[8]], dim=1)
        return rel_pos, rel_depth_images
...
def test_policy(args):
    ...
    # Initialize state and next state.  These only hold 3 depth images and 8 positions as tensors
    #stateObj = StateClass(8,3)
    #nextStateObj = StateClass(8,3)
    ...
    #stateObj = StateClass(posT, depthT)     # Trying to make this a class to calculate memory size easily?
    ...
    for i in range(0, 50000):
        ...
        pos, depth_image, rewards, resets = observe_env(env, actionObj) # Observe reward and environment
        stateQObj.addObservation(pos, depth_image)      # Add observations to the queue
        nextPosT, nextDepthT = stateQObj.nextState()      # This returns the latest 8 position observations, and latest 3 depth observations as tensors
